...
def run_pipeline(model_type, csv_path, text_col, label_col, model_dir_base="./modelos_dl", use_saved=True, output_image_dir="."):
    ...
    # Vocabulario
    vocab = None
    if use_saved and os.path.exists(vocab_path):
        print("Cargando vocabulario guardado...")
        try:
            with open(vocab_path, 'rb') as f:
                vocab = pickle.load(f)
        except Exception as e:
            print(f"Error cargando vocabulario: {e}. Se reconstruirá.")
    ...
    if use_saved and os.path.exists(model_path):
        ...
        try:
            ...
            print(f"Error cargando modelo (posible incompatibilidad): {e}")
            print("Re-entrenando modelo...")
            # Re-entrenamiento si falla la carga
    ...
    plt.title(f'Matriz de Confusión Normalizada - {model_type}')
    plt.xlabel('Predicción')
    plt.ylabel('Realidad')
    plt.savefig(os.path.join(output_image_dir, f"confusion_normalized_{model_type}.png"))
    plt.close()
